	...
	const char *name)
{
	...
	do {
		...
		for (di = dir_info; opt_do_run && di; di = di->next) {
			 ...
			 *  avoid this.
			 */
			/* Added by Zoran Stojsavljevic November 12th, 2018 */
			static const char *ttt0 = "/dev/ttyS0";
			static const char *ttt1 = "/dev/ttyS1";
			static const char *ttt4 = "/dev/ttyS4";
			int result;
			// result = (strcmp(ttt0, di->path) && strcmp(ttt1, di->path) && strcmp(ttt4, di->path));
			result = strcmp(ttt4, di->path);			
			    if (0 == result) {
			        // usleep(300000);
				printf("Opening the serial driver %s\n", di->path);
				// End Zoran
			    if (!euid) {
				fd = open(di->path, O_RDONLY | O_NONBLOCK);
				printf("Opened file handle is %x\n", fd); // Zoran
				if (fd < 0) {
					di->noaccess = true;
					continue;
				}
				...
				if ((fstat(fd, &buf) < 0) &&
				    (errno != ENOMEM)) {
					di->ignore = true;
					printf("*** Closing file handle %x with error %x ***\n", fd, errno); // Zoran
					(void)close(fd);
					continue;
				}
